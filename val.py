...
def main(args):
    ...
    def collate_fn(batch):
        ...
        for sample in batch:
            if len(sample) == 13:
                ...
                audo,audio_aug, label = sample
                d = {
                    "mfcc":      None,      "mfcc_aug": None,
                    "audio":     audo,"audio_aug":audio_aug,      "video":     None,
    ...
    audio_dataset = AudioDataset(path, partition="test", take_datasets="1,2", audio_preturb={})
    dev_sampler = torch.utils.data.distributed.DistributedSampler(audio_dataset)
    audio_data_loader = DataLoader(audio_dataset, batch_size=args.batch_size,
                          sampler=dev_sampler,
    ...
    # fakeavceleb_test_dataset = HashFormerDataset(path, partition="test", take_datasets="REAL_TEST,FakeAVCeleb", visual_preturb={
    #                         "saturation": 5,
    #                         "contrast": None,
    #                         "blockwise": None,
    #                         "gaussian_noise": None,
    ...
    # deepspeak_test_dataset = HashFormerDataset(path, partition="test", take_datasets="1,2")
    # dev_sampler = torch.utils.data.distributed.DistributedSampler(deepspeak_test_dataset)
    # deepspeak_eval = DataLoader(deepspeak_test_dataset, batch_size=args.batch_size,
    #                       sampler=dev_sampler,
    #                       num_workers=args.num_workers,
    #                       pin_memory=True,
    #                       drop_last=True,
   ...
   # ----------------  optimiser  ----------------
    ...
    for loader_info in [
                        # {"loader":deepspeak_eval,"name":"deepspeak_eval"},
                        {"loader":audio_data_loader,"name":"AVSpoof"},
    ...
    # break
        ...
        with torch.no_grad():
            ...
            if writer is not None:
                writer.add_scalar(f"{loader_name}/Val/AvgLoss", val_loss, 0)
                writer.add_scalar(f"{loader_name}/Val/Accuracy", val_accuracy, 0)
                writer.add_scalar(f"{loader_name}/Val/ROC_AUC", roc_auc_val, 0)
                writer.add_scalar(f"{loader_name}/Val/AUC_PR", auc_pr_val, 0)
                # writer.add_scalar("Val/AUC_R", auc_r_val, 0)
                writer.add_scalar(f"{loader_name}/Val/AP", ap_score_val, 0)
                writer.add_scalar(f"{loader_name}/EER/val", val_eer, 0)
