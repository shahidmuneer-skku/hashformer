...
def main(args):
    ...
    # Create the dataset
    path = args.base_dir
    train_dataset = HashFormerDataset(path, partition="train", take_datasets="TIMIT_HQ,TIMIT_LQ,DFDC,FakeAVCeleb,1")
...
# ,train_dataset,faceswap_video_train_dataset
    ...
    df_timit_test_dataset = HashFormerDataset(path, partition="test", take_datasets="REAL_TEST,TIMIT_HQ,KoDF")
        ...
        for threashold in [0.2,0.4,0.5,0.7]:
            ...
            # break
                ...
                with torch.no_grad():
                    ...
                    if writer is not None:
                        writer.add_scalar(f"Val/AvgLoss", val_loss, epoch)
                        writer.add_scalar(f"{loader_name}/{threashold}/Val/Accuracy", val_accuracy, epoch)
                        writer.add_scalar(f"Val/ROC_AUC", roc_auc_val, epoch)
                        writer.add_scalar(f"Val/AUC_PR", auc_pr_val, epoch)
                        # writer.add_scalar("Val/AUC_R", auc_r_val, epoch)
                        writer.add_scalar(f"Val/AP", ap_score_val, epoch)
                        writer.add_scalar(f"EER/val", val_eer, epoch)
