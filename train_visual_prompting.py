...
from models.baselines.visualPromptModel import MultiModalPromptDetector
...
def main(args):
    ...
    # Create the dataset
    path = args.base_dir
    # train_dataset = HashFormerDataset(path, partition="train", take_datasets="REAL_TEST,TIMIT_LQ")
    ...
    full_train = HashFormerDataset(path, partition="train", take_datasets="REAL_TEST,TIMIT_LQ")
    g = torch.Generator().manual_seed(42)   # keep the same seed for repeatability
...
# ,train_dataset,faceswap_video_train_dataset
    ...
    pretraining = False
    num_classes = full_train.class_numbers
    # model = HashFormer(pretraining=pretraining)
    model = MultiModalPromptDetector()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for loader_info in [{"loader":train_loader,"name":"VideoAudio"}]:
            ...
            for i, batch in enumerate(tqdm(data_loader, desc=f"Epoch {epoch + 1}/{args.epochs} {loader_name}")):
                ...
                # batch = random_drop(batch, drop_prob=0.1, keys=("mfcc", "video","landmarks"))
                for mod in batch:
                ...
                if isinstance(raw, dict):
                    ...
                    raise RuntimeError(f"loss returned an unexpected type: {type(raw)}")
                ...
                pos_samples.append(
                    pred[(label == 1).to(pred.device)].detach().cpu().numpy()
                )
                
                neg_samples.append(
                    pred[(label == 0).to(pred.device)].detach().cpu().numpy()
                )
        ...
        for loader_info in [
                            # {"loader":val_dataloader,"name":"deepspeak_eval"},
                            ...
                            {"loader": val_dataloader, "name": "DeepSpeak"},
        ...
        # break
            ...
            with torch.no_grad():
                for i, batch in enumerate(tqdm(data_loader, desc="Validation")):
                    ...
                    output = model(**batch)
                    classification_logits = output["f_score"]
