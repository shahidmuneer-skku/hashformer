...
def main(args):
    ...
    # Create the dataset
    path = args.base_dir
    train_dataset = HashFormerDataset(path, partition="train", take_datasets="1,2,FakeAVCeleb,v_real")
    ...
    faceswap_val_video_dataset = FaceswapVideoDataset(path, partition="test", take_datasets=["FaceSwap"])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for loader_info in [{"loader":train_loader,"name":"VideoAudio"}]:
            ...
            for i, batch in enumerate(tqdm(data_loader, desc=f"Epoch {epoch + 1}/{args.epochs} {loader_name}")):
                ...
                # batch = random_drop(batch, drop_prob=0.1, keys=("audio","video","landmarks"))
                batch = random_drop(batch, drop_prob=0.1, keys=("mfcc","landmarks"))
                for mod in batch:
        ...
        for loader_info in [
                            ...
                            {"loader": deepspeak_val, "name": "DeepSpeak"},
                            # {"loader": faceswap_video_val_loader, "name": "Faceofensics"},
